        ...
        for (i=0; i<3; i=i+1)begin
            ...
            always @(posedge clk)begin
                if (rst)begin
                    ...
                    shifted_pixel[i] <= $signed(fir_pixel[i]) >>> scaler; // preserves signed MSB, then scales down the magnitude of the 
                end
            end
            always @(posedge clk)begin
                if (rst)begin
                    ...
                    if (ob)begin
                        //6S965 Student: CHANGE ME!!!
                        clipped_pixel[i] <= (shifted_pixel[i] > 32'sd127) ? 32'sd127 : (shifted_pixel[i] < -32'sd128) ? -32'sd128 : shifted_pixel[i];
                    end else begin
                        //6S965 Student: CHANGE ME!!!
                        clipped_pixel[i] <= (shifted_pixel[i] > 32'sd255) ? 32'sd255 : (shifted_pixel[i] < 32'sd0) ? 0 : shifted_pixel[i];
                    end
                end
            end
        end
